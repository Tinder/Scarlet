...
fun ValueAssert<EventSourceEvent.OnMessageReceived>.containingText3(expectedText: String): ValueAssert<EventSourceEvent.OnMessageReceived> {
    return assert {
        assertThat(message).isInstanceOf(Message.Text::class.java)
        val (text) = message as Message.Text
        assertThat(text).isEqualTo(expectedText)
...
fun ValueAssert<SocketIoEvent.OnMessageReceived>.containingText4(expectedText: String): ValueAssert<SocketIoEvent.OnMessageReceived> {
    return assert {
        assertThat(message).isInstanceOf(Message.Text::class.java)
        val (text) = message as Message.Text
        assertThat(text).isEqualTo(expectedText)
...
fun ValueAssert<SocketIoEvent.OnMessageReceived>.containingBytes4(expectedBytes: ByteArray): ValueAssert<SocketIoEvent.OnMessageReceived> {
    return assert {
        assertThat(message).isInstanceOf(Message.Bytes::class.java)
        val (bytes) = message as Message.Bytes
        assertThat(bytes).isEqualTo(expectedBytes)
